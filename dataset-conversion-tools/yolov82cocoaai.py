...
def yolov8_to_coco(data_yaml_path, output_dir, num_samples=None):
    ...
    handling of negative bounding box coordinates, rounding of coordinates
    and area to 4 significant figures, handling of multiple annotations
    per image, and images with empty label files. Ensures unique image IDs
    across train and val splits.
    ...
    img_id_offset = 0  # Initialize image ID offset
    ...
    for split in ['train', 'val']:
        ...
        for filename in os.listdir(img_dir):
            if filename.endswith(('.jpg', '.jpeg', '.png')):
                img_id = len(images) + img_id_offset  # Add offset to ensure unique IDs
                    ...
                    with open(label_file, 'r') as lf:
                        for line in lf:
                            ...
                            try:
                                ...
                                annotations.append({
                                    'id': ann_id,
                                    'image_id': img_id,  # Use the unique image ID
        ...
        img_id_offset += len(images)  # Update offset for the next split
